...
test("skip interval from the settings page is used by the shortcuts", async ({ context, extensionId }) => {
  ...
  await expect(settings.locator("#skip-options").getByRole("radio", { name: "30s" })).toHaveAttribute("aria-checked", "true");
...
test("keybind editor: record a key; taking another action's key moves it", async ({ context, extensionId }, testInfo) => {
  const settings = await context.newPage();
  ...
  await expect(settings.locator(".shortcut", { hasText: "Fullscreen" }).locator(".binding-empty")).toHaveText("None");
  await testInfo.attach("settings", { body: await settings.screenshot({ fullPage: true }), contentType: "image/png" });
