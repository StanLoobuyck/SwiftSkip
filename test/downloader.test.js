...
test("keeps segments in playlist order even when they arrive out of order", async () => {
  const c = collect();
  const result = await runHlsDownload({
...
test(
  "real MPEG-TS is converted to a playable MP4",
  { skip: !hasFfmpeg() && "ffmpeg not installed" },
  async () => {
    // Point a playlist at the real segments ffmpeg made.
    const segs = readdirSync(realTsDir)
