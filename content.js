...
(function () {
  ...
  // ─── Config ────────────────────────────────────────────────────────────────
  const config = { skipSeconds: 10, enabled: true };
  ...
  function isLectureManifestUrl(url) {
    if (!url || typeof url !== "string") return false;
    const lowerUrl = url.toLowerCase();

    return (
      lowerUrl.includes(".m3u8") ||
      (lowerUrl.includes("playmanifest") &&
        lowerUrl.includes("format/applehttp"))
    );
  }
  ...
  function injectDownloadButton() {
    if (!detectedLectureUrl || downloadDismissed) return;
